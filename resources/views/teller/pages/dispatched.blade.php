...
<style>
    ...
    @if(session('error_alert'))
        <div id="error-alert" class="error_alert">
            <strong></strong> {{ session('error_alert') }}
        </div>
        <script>
            document.addEventListener("DOMContentLoaded", function () {
                let alertBox = document.querySelector(".error_alert");
...
<div id="claimConfirmationModal" class="modal">
    <div class="modal-content">
        <h2>Are you sure?</h2>
        <p>Once submitted, you won't be able to undo this action.</p>
        <div class="modal-buttons">
            <button id="confirmClaimSubmit" class="confirm-btn">Submit</button>
            <button id="cancelClaimSubmit" class="cancel-btn">Cancel</button>
        </div>
    </div>
</div>

<div id="unclaimConfirmationModal" class="modal">
    <div class="modal-content">
        <h2>Are you sure?</h2>
        <p>Once submitted, you won't be able to undo this action.</p>
        <div class="modal-buttons">
            <button id="confirmUnclaimSubmit" class="confirm-btn">Submit</button>
            <button id="cancelUnclaimSubmit" class="cancel-btn">Cancel</button>
        </div>
    </div>
</div>
 
<div id="claimAlert" class="error_alert" style="display:none;">
    Please select at least one shipment
</div>
<div id="unclaimAlert" class="error_alert" style="display:none;">
    Please select at least one shipment 
</div>
...
@section('content')
    <div class="submit-container">
        <form id="claimForm" method="POST" action="{{ route('claim.shipments') }}">
            @csrf
            <button type="button" class="submit-btn" id="claimBtn">
                <i class="fas fa-check-circle"></i> Claim
            </button>
            <input type="hidden" name="user_ids" id="claimSelectedShipments">
        </form>
        <form id="unclaimForm" method="POST" action="{{ route('unclaim.shipments') }}">
            @csrf
            <button type="button" class="submit-btn" id="unclaimBtn">
                <i class="fa fa-times-circle"></i> Unclaim
            </button>
            <input type="hidden" name="user_ids" id="unclaimSelectedShipments">
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const claimModal = document.getElementById('claimConfirmationModal');
        const confirmClaimSubmit = document.getElementById('confirmClaimSubmit');
        const cancelClaimSubmit = document.getElementById('cancelClaimSubmit');
        const claimForm = document.getElementById('claimForm');
        const claimAlert = document.getElementById('claimAlert');
        const claimSelectedShipments = document.getElementById('claimSelectedShipments');
    
        claimBtn.addEventListener('click', function (event) {
            event.preventDefault();
            const selected = getSelectedShipments();
            if (selected.length === 0) {
                showAlert(claimAlert);
            } else {
                claimModal.classList.add('show');
            }
        });
    
        confirmClaimSubmit.addEventListener('click', function () {
            claimSelectedShipments.value = getSelectedShipments().join(',');
            claimForm.submit();
        });
    
        cancelClaimSubmit.addEventListener('click', function () {
            closeModal(claimModal);
        });
    
        // Unclaim
        const unclaimBtn = document.getElementById('unclaimBtn');
        const unclaimModal = document.getElementById('unclaimConfirmationModal');
        const confirmUnclaimSubmit = document.getElementById('confirmUnclaimSubmit');
        const cancelUnclaimSubmit = document.getElementById('cancelUnclaimSubmit');
        const unclaimForm = document.getElementById('unclaimForm');
        const unclaimAlert = document.getElementById('unclaimAlert');
        const unclaimSelectedShipments = document.getElementById('unclaimSelectedShipments');
    
        unclaimBtn.addEventListener('click', function (event) {
            event.preventDefault();
            const selected = getSelectedShipments();
            if (selected.length === 0) {
                showAlert(unclaimAlert);
            } else {
                unclaimModal.classList.add('show');
            }
        });
    
        confirmUnclaimSubmit.addEventListener('click', function () {
            unclaimSelectedShipments.value = getSelectedShipments().join(',');
            unclaimForm.submit();
        });
    
        cancelUnclaimSubmit.addEventListener('click', function () {
            closeModal(unclaimModal);
        });
    
        // Modal close on outside click
        window.addEventListener('click', function (event) {
            if (event.target === claimModal) closeModal(claimModal);
            if (event.target === unclaimModal) closeModal(unclaimModal);
        });
...
<script>
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            fetch(`/teller/pages/details/dispatched/${shipmentId}`)
