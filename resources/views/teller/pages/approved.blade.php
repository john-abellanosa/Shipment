...
<script>
    function openCancelModal() {
        ...
        if (selectedUserIds.length === 0) {
            ...
            alertBox.innerHTML = `Please select at least one shipment before cancelling.`;
            ...
            document.getElementById('hidden-cancel-user-ids').value = JSON.stringify(selectedUserIds);
...
<script>
        ...
        if (selectedUserIds.length === 0) {
            const alertBox = document.createElement('div');
            alertBox.className = 'error_alert';
            alertBox.innerHTML = `Please select at least one shipment before proceeding.`;
            document.body.appendChild(alertBox);
            ...
            document.getElementById('hidden-ready-user-ids').value = JSON.stringify(selectedUserIds);
...
<div id="confirmationModal" class="modal">
    <div class="modal-content">
        ...
        <form id="readyDispatchForm" method="POST" action="{{ route('teller.pages.approved_shipment') }}">
            @csrf
            <input type="hidden" name="user_ids" id="hidden-ready-user-ids">
            <div class="modal-buttons">
                <button type="submit" class="confirm-btn">Submit</button>
                <button type="button" class="cancel-btn" onclick="closeReadyModal()">Cancel</button>
            </div>
        </form>
    </div>
</div>

<div id="cancel-modal" class="modal-new">
    <div class="modal-content-new">
        <h3>Enter Reason for Cancellation</h3>
        <form id="cancelForm" method="POST" action="{{ route('cancel.shipments') }}">
            @csrf
            <input type="hidden" name="user_ids" id="hidden-cancel-user-ids">
            <div class="input-container">
                <textarea name="cancel_reason" placeholder="State the reason for cancellation..." oninput="autoResize(this)" required></textarea>
            </div>

            <div class="modal-buttons-new">
                <button type="submit" class="confirm-btn-new">Submit</button>
                <button type="button" class="cancel-btn-new" onclick="closeCancelModal()">Cancel</button>
            </div>
        </form>
    </div>
</div>
...
@section('content')
    <form method="POST" action="#">
        ...
        <div class="submit-container">
            <button type="button" class="submit-btn" onclick="openReadyModal()">
                <i class="fa fa-truck"></i> Ready for Dispatch
            </button>
        
            <button type="button" class="cancel-button" onclick="openCancelModal()">
                <i class="fa fa-times-circle"></i> Cancel Shipment
            </button>
        </div>
...
<script>
    ...
    document.querySelectorAll('.view-btn').forEach(button => {
        button.addEventListener('click', function () {
            ...
            fetch(`/teller/pages/details/approved/${shipmentId}`)
