...
<body>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            let errorAlertPhone = document.getElementById('error-alert-phone');
            let errorAlertEmail = document.getElementById('error-alert-email');
            ...
            if (successAlert) hideAlert(successAlert);
            if (errorAlert) hideAlert(errorAlert);
            if (errorAlertPhone) hideAlert(errorAlertPhone);
            if (errorAlertEmail) hideAlert(errorAlertEmail);
        });
    </script>
...
-->
    ...
    <div id="branchModal" class="modal1">
        <div class="modal-content1">
            <div class="modal-header1">
                <div class="inner">
                    <h2 class="modal-title1">Add New Branch</h2>
            ...
            <form action="{{ route('branch.store') }}" method="POST" class="modal-body1">
                @csrf
                <div class="form-group">
                    <label for="province">Province</label>
                    <input type="text" id="province" name="province" required placeholder="Enter Branch Province">
                </div>
                <div class="form-group">
                        ...
                        <tbody>
                            @forelse ($branches as $index => $branch)
                                <tr data-status="{{ $branch->status }}">
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            @if ($branch->status == 'open')
                                                <a href="#" class="status-button-open-button"
                                                    onclick="toggleBranchStatus('{{ $branch->id }}', 'close')">
                                                    <i class="fas fa-check-circle"></i><span>Open</span>
                                                </a>
                                            @else
                                                <a href="#" class="status-button-close-button"
                                                    onclick="toggleBranchStatus('{{ $branch->id }}', 'open')">
                                    ...
                                    <td class="actions">
                                        <a href="javascript:void(0);" class="edit-btn tooltip"
                                            data-id="{{ $branch->id }}" data-province="{{ $branch->province }}"
                                        ...
                                        <button class="delete-btn tooltip" onclick="openModal({{ $branch->id }})">
                            ...
                            @empty
                                <tr>
                                    <td colspan='8' style='text-align:center;'>No Branch found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                @if ($branches->isNotEmpty())
                    <div class="pagination-container">
                        <div class="pagination-links">
                            {{ $branches->onEachSide(1)->links('vendor.pagination.custom') }}
                        </div>

                        <div class="results-summary">
                            Showing {{ $branches->count() }} out of {{ $branches->total() }} Branches
                        </div>
                    </div>
                @endif
            </div>
    ...
    <script>
        function toggleBranchStatus(branchId, status) {
            fetch(`/branch/update-status/${branchId}`, {
                    method: "PUT",
                    headers: {
    ...
    <div id="editModal" class="modal1">
        <div class="modal-content1">
            <div class="modal-header1">
                <div class="inner">
                    <h2 class="modal-title1">Update Branch</h2>
            ...
            <form id="editForm" method="POST" class="modal-body1">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="edit-contact_person" name="contact_person" required>
                </div>
                <div class="form-group">
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            editBtns.forEach(btn => {
                btn.addEventListener("click", function() {
                    ...
                    document.getElementById("edit-province").value = this.getAttribute(
                        "data-province");
                    document.getElementById("edit-address").value = this.getAttribute(
                        "data-address");
                    document.getElementById("edit-contact_person").value = this.getAttribute(
                        "data-contact_person");
                    document.getElementById("edit-phone").value = this.getAttribute("data-phone");
                    document.getElementById("edit-email").value = this.getAttribute("data-email");

                    form.action = `/branch/update/${id}`;
